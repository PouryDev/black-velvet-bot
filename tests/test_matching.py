...
async def test_fifo_picks_first_compatible(tmp_path: Path) -> None:
    ...
    await db.match_or_enqueue(
        user_id=10, gender="trans", position="vers", wanted_gender="male", wanted_position="top"
    )
    await db.match_or_enqueue(
        user_id=11, gender="trans", position="vers", wanted_gender="male", wanted_position="top"
    )
    partner = await db.match_or_enqueue(
        user_id=12, gender="male", position="top", wanted_gender="trans", wanted_position="vers"
    )
    assert partner is not None
